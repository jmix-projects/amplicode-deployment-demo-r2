import {getScreenKey, ScreenMetaProvider, screenStore, useScreens} from "@amplicode/react-core";
import {BreadcrumbsArea, TabHeading} from "@amplicode/react-antd";
                    ...
                    <Tabs activeKey={activeBreadcrumb?.key} renderTabBar={() => <></>}>
                        {tabs[index].breadcrumbs.map(breadcrumb => (
                            <Tabs.TabPane key={breadcrumb.key}>
                                <ScreenMetaProvider tab={tab} breadcrumb={breadcrumb}>
                                    {breadcrumb.content}
                                </ScreenMetaProvider>
                            </Tabs.TabPane>
                        ))}
