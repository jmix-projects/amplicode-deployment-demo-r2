...
const ROUTE = "pet-type-list";

const PET_TYPE_LIST = gql(/* GraphQL */ `
  query petTypeList {
    petTypeList {
      id
      name
...
const DELETE__PET_TYPE = gql(/* GraphQL */ `
  mutation delete_PetType($id: Long!) {
    delete_PetType(id: $id)
  }
`);

export const PetTypeList = observer(({ onSelect }: EntityListScreenProps) => {
  ...
  const [executeDeleteMutation] = useMutation(DELETE__PET_TYPE);
  ...
  const openEditor = useCallback(
    (id?: string) => {
      const params: OpenInBreadcrumbParams = {
        breadcrumbCaption: intl.formatMessage({ id: "screen.PetTypeDetails" }),
        component: PetTypeDetails
      };
      if (id != null) {
        params.props = { id };
      }
...
function getUpdateFn(e: any) {
  return (cache: ApolloCache<any>) => {
    cache.modify({
      fields: {
        petTypeList(existingRefs, { readField }) {
